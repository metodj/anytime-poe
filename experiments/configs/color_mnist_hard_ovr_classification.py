...
def get_config() -> config_dict.ConfigDict:
    ...
    config.dataset_name = "ColorMNIST"
    ...
    num_train, _ = train_val_split_sizes(
        METADATA["num_train"][config.dataset_name], config.val_percent
    )
    ...
    config.model.net_type = "ConvNetRGB"

    config.β_schedule = config_dict.ConfigDict()
    config.β_schedule.name = "sigmoid"
    config.β_schedule.start = 2.0
    config.β_schedule.end = 16.0
    ...
    config.alpha_schedule.start = 0.0
    config.alpha_schedule.end = 1.0
