...
class Hard_OvR_Ens(nn.Module):
    ...
    def setup(self):
        if self.net_type == "ResNetMLP":
            ...
            raise ValueError()
        weights = self.param("weights", self.weights_init, (self.size,))
    ...
    def __call__(
        ...
        ens_logits = jnp.stack(
            [net(x, train=train) for net in self.nets], axis=0
        )  # (Μ, Ο)
        ...
        if self.ll_type == "ovr":
            ...
            product_logprob = partial(
                product_logprob_softmax, ens_logits=ens_logits, probs=probs
            )
        else:
            raise ValueError()
        ...
        loss = -1.0 * prod_ll + Z_multiplier * jnp.log(Z + 1e-36)
...
def make_Hard_OvR_Ens_loss(
    ...
    def batch_loss(params, state, rng):
        ...
        return agg(loss_for_batch, axis=0), (
            new_state,
            agg(err_for_batch, axis=0),
            agg(prod_ll_for_batch, axis=0),
            agg(members_ll_for_batch, axis=0),
        )
...
def make_Hard_OvR_Ens_toy_plots(
    ...
    hard_ovr_params, hard_ovr_model_state = (
        hard_ovr_state.params,
        hard_ovr_state.model_state,
    )
    ...
    pred_fun = partial(
        ...
        train=False,
        return_ens_preds=True,
        β=hard_ovr_state.β,
        method=hard_ovr_model.pred,
    )
    preds, _ = jax.vmap(pred_fun, out_axes=(0, 1), in_axes=(0,), axis_name="batch")(xs)
    ...
    for i in range(n_class):
        axs.pcolormesh(
            xx, yy, preds[:, i].reshape(xx.shape), alpha=0.25, cmap=colormaps[i]
        )
    ...
    for i in range(n_class):
        idxs = y_train == i
        axs.plot(
            X_train[idxs, 0], X_train[idxs, 1], markers[0], c=f"C{i}", alpha=1, ms=1
        )
...
def make_Hard_OvR_Ens_MNIST_plots(
    ...
    preds = jax.vmap(pred_fun, axis_name="batch")(xs)
    ...
    for idx in range(n_plots):
        ...
        axs[idx].set_title(
            f"{jnp.argmax(preds[idx])} – {jnp.max(preds[idx]):.4f} ({ys[idx]} – {preds[idx][ys[idx]]:.4f})",
            fontsize=12,
        )
        axs[idx].set_xticks([])
        axs[idx].set_yticks([])
